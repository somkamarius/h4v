...
export function IntroPage({ setDisplayGame, setInitialMessage, setUpperCode }: Props) {
    ...
    return (
        <div className={` bg-gray-800 min-h-screen transition-all duration-[3000ms] ${fade ? "opacity-0" : "opacity-100"}`} >
            <div className={`pt-20 mx-20 transition-all duration-[2000ms] ${!fade ? "opacity-100" : "opacity-0"}`}>
                ...
                <button onClick={handleSubmit} type="submit" className={`mt-4  transition-all duration-[3000ms]  ${!fade ? 'text-grey-800' : 'text-purple-800'}  font-bold bg-white`}>
                    Begin the journey
                </button>
            </div>
        </div >
